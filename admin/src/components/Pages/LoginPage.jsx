...
export default function LoginPage(){
    ...
    const validate = () => {
        ...
        if (!password) fieldErr.password = "Enter password"
    ...
    const onSubmit = async (e) => {
        ...
        try{
            const res = await fetch(new URL("/auth/login", "http://localhost:3000").toString(), {
                method: "POST",
        ...
        if(!res.ok) {
            ...
            const msg = data?.message || data?.error || "Login error"
            const f = {}
        ...
        if(!token || !user) {
            setErrors({ form: "Malformed response: token or user missing", fields: {} }) 
            return
        }
